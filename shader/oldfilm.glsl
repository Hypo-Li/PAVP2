...
float filmDirt(vec2 pp, float time) 
{
	...
	vec3 nse2 =
		textureLod(iTexture0,.1*nseLookup2.xy,0.).xyz +
		textureLod(iTexture0,.01*nseLookup2.xy,0.).xyz +
		textureLod(iTexture0,.004*nseLookup2.xy+0.4,0.).xyz;
	float thresh = .6;
	...
	float seed = textureLod(iTexture0,vec2(time*.35,time),0.).x;
...
{
    vec2 qq = -1.0 + 2.0 * iUV;
    qq.x *= iSize.x / iSize.y;
    float time = limitFPS(iTime, FPS);

	vec4 jumpCutData = jumpCut(time);
    vec4 dirt = vec4(filmDirt(qq, time + jumpCutData.w));     
    vec4 image = texture(iFrame, moveImage(iUV, time));   
    vec4 vig = vignette(iUV, time);
    
    vec3 color = vec3(image * dirt * vig);
    fragColor = vec4(blackAndWhite(color), 1.0);
}